...
//! (`lex → parse → check → run`) over `.adder` fixture programs.
//!
...
fn showcase_prints_9_0() {
    let o = run_fixture("examples/eval.adder");
...
fn missing_match_arm_is_a_compile_error() {
    let o = run_fixture("examples/errors/exhaustive_missing.adder");
...
fn nullable_used_as_non_null_is_a_compile_error() {
    let o = run_fixture("examples/errors/null_unnarrowed.adder");
...
fn narrowed_nullable_compiles_and_runs() {
    let o = run_fixture("examples/narrowed.adder");
...
fn val_reassignment_is_rejected() {
    let o = run_fixture("examples/errors/val_reassign.adder");
...
fn non_bool_condition_is_rejected() {
    let o = run_fixture("examples/errors/non_bool_cond.adder");
...
fn loop_accumulator_mutates_outer_binding() {
    let o = run_fixture("examples/errors/accumulator.adder");
